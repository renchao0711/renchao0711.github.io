...
• Source0 只包含了一个回调（函数指针），它并**不能主动触发事件**。使用时，你需要先调用 CFRunLoopSourceSignal(source)，将这个 Source 标记为待处理，然后手动调用 CFRunLoopWakeUp(runloop) 来唤醒 RunLoop，让其处理这个事件。即需要手动唤醒runloop。
